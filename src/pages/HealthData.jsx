...
const HealthData = () => {
  ...
  return (
    <React.Fragment>
     ...
     <div className="mb-1 flex flex-row justify-between mx-4 my-6">
        <h1 className='text-4xl text-blue-700 font-bold'>Health Data</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
        <div className="p-4 border border-gray-200 rounded-lg h-[150px] bg-blue-500 text-white">
          ...
          <p className="text-white text-3xl font-bold">{filteredData.length > 0 ? filteredData[0].calories : '-'}</p>
        </div>
        <div className="p-4 border border-gray-200 rounded-lg bg-blue-500 text-white">
          <h2 className="text-white mb-1 text-2xl font-bold">Body Water</h2>
          <FaHandHoldingWater size={35} />
          <p className="text-white text-3xl font-bold">{filteredData.length > 0 ? filteredData[0].bodyWater : '-'}</p>
        </div>
        <div className="p-4 border border-gray-200 rounded-lg bg-blue-500 text-white">
          <h2 className="text-white mb-2 text-2xl font-bold">Exercises Duration</h2>
          <FaPersonRunning size={35} />
          <p className="text-white text-3xl font-bold">{filteredData.length > 0 ? filteredData[0].exercisesDuration : '-'}</p>
        </div>
        <div className="p-4 border  border-gray-200 rounded-lg bg-blue-500 text-white">
          <h2 className="text-white mb-2 text-2xl font-bold">Heart Rate</h2>
          <FaHeartbeat  size={35}/>
          <p className="text-white text-3xl font-bold">{filteredData.length > 0 ? filteredData[0].heartRate : '-'}</p>
        </div>
        </div>
        <h1 className="text-2xl text-blue-600 font-semibold my-4">Vital Signs</h1>
        ...
        <div className='flex flex-col ml-1'>
          ...
          <p className="text-white text-3xl font-bold">{filteredData.length > 0 ? filteredData[0].bloodPressure : '-'}</p>
          </div>
        </div>
        <div className="p-4 border flex flex-row border-gray-200 rounded-md bg-blue-500">
        ...
        <div className='flex flex-col ml-1'>
          <h2 className="text-white mb-2 text-2xl font-bold">Heart Rate</h2>
          <p className="text-white text-3xl font-bold">{filteredData.length > 0 ? filteredData[0].respLevel : '-'}</p>
          </div>
        </div>
        ...
        <MdBloodtype size={50} className='mt-1 text-red-700'/>
          ...
          <p className="text-white text-3xl font-bold">{filteredData.length > 0 ? filteredData[0].stressLevel : '-'}</p>
          </div>
        </div>
        <div className="p-4 border flex flex-row border-gray-200 rounded-md bg-blue-500">
